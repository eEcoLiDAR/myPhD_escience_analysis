...
lidar_metrics_masked <- mask(lidar_metrics,formask)
...
# predict
predLC <- predict(lidar_metrics_masked, model=modelRF, na.rm=TRUE)
...
writeRaster(predLC, filename="classified_reed.tif", format="GTiff",overwrite=TRUE)