...
bird_data_onebird_onlypres=bird_data_onebird[ which(bird_data_onebird$number>0),]
...
p1=ggplot() + 
  ...
  coord_equal() +
  geom_point(data=bird_data_onebird_onlypres_2013, aes(x=x_point,y=y_point,color=factor(month)),inherit.aes = FALSE) +
...
p2=ggplot() + 
  ...
  geom_path(color="white") +
  coord_equal() +
  geom_point(data=bird_data_onebird_onlypres_2014, aes(x=x_point,y=y_point,color=factor(month)),inherit.aes = FALSE) +
...
p3=ggplot() + 
  ...
  geom_path(color="white") +
  coord_equal() +
  geom_point(data=bird_data_onebird_onlypres_2015, aes(x=x_point,y=y_point,color=factor(month)),inherit.aes = FALSE) +
...
p4=ggplot() + 
  ...
  geom_path(color="white") +
  coord_equal() +
  geom_point(data=bird_data_onebird_onlypres_2016, aes(x=x_point,y=y_point,color=factor(month)),inherit.aes = FALSE) +
...
#bird_groupby=ddply(bird_data_onebird_onlypres,~pointid+x_point+y_point+number+species,summarise,sum=sum(number))
bird_groupby=ddply(bird_data_onebird_onlypres,~kmsquare+x_point+y_point+number+species,summarise,sum=sum(number))
...
p5=ggplot() + 
  ...
  geom_point(data=bird_groupby, aes(x=x_point,y=y_point,size=sum,color="red"),inherit.aes = FALSE)

ggsave(paste(substr(filename, 1, nchar(filename)-4),'_grouped_onlypres.jpg',sep=''),p5)

#Presence-absence
#observationmap=ddply(bird_data_onebird,~pointid+x_point+y_point+species,summarise,sum=sum(number))
observationmap=ddply(bird_data_onebird,~kmsquare+x_point+y_point+species,summarise,sum=sum(number))
...
coordinates(observationmap)=~x_point+y_point
...
p7=ggplot() + 
  ...
  geom_path(color="white") +
  coord_equal() +
  geom_point(data=observationmap_filt_df, aes(x=x_point,y=y_point,size=sum,color=presence),inherit.aes = FALSE) +
